...
def load_test_cases():
    """Tải 5 test cases từ config/test_cases.json."""
    base_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    config_path = os.path.join(base_dir, "config", "test_cases.json")
...
def run_baseline_chatbot(user_query: str, provider):
    """Chatbot baseline không gọi Tool."""
    print(f"\n💬 [CHATBOT BASELINE] Câu hỏi: {user_query}")
    response = provider.generate(user_query, system_prompt=CHATBOT_BASELINE_PROMPT)
    print(f"🤖 Chatbot phản hồi:\n{response}")


def build_next_input(original_query: str, history: list) -> str:
    """Đưa Observation của các bước trước trở lại LLM để tiếp tục ReAct."""
    return (
        f"Yêu cầu ban đầu của người dùng:\n{original_query}\n\n"
        f"Lịch sử Action/Observation đã thực hiện:\n"
        f"{json.dumps(history, ensure_ascii=False, indent=2)}\n\n"
        "Hãy tiếp tục xử lý yêu cầu ban đầu. "
        "Nếu cần thêm Tool thì tạo Tool Call tiếp theo; "
        "nếu đã đủ dữ liệu thì trả lời Final Answer bằng text."
    )
...
def run_react_agent(user_query: str, provider, mcp_server: MCPAcademicServer) -> list:
    ...
    while step < MAX_ITERATIONS:
        ...
        if llm_response.get("type") == "tool_call":
            ...
            # Quan trọng: Observation quay lại LLM để có thể thực hiện multi-step.
            current_input = build_next_input(user_query, tool_history)
            continue

        # Provider trả về cấu trúc không mong đợi.
        error_message = f"LLM trả về response type không hợp lệ: {llm_response.get('type')}"
        ...
        final_message = "Agent đã đạt giới hạn số vòng lặp trước khi hoàn tất yêu cầu."
...
if __name__ == "__main__":
    print("==========================================================")
    print("📦 DAY 03 LAB - SUPPLY CHAIN REACT AGENT")
    print("==========================================================")
    ...
    if "--interactive" in sys.argv:
        print("🎮 [INTERACTIVE MODE] Trò chuyện với Supply Chain Agent")
        print("💡 Ví dụ:")
        print("   - Tra cứu: 'Kiểm tra đơn hàng ORD2026001'")
        print("   - Cập nhật: 'Cập nhật ORD2026001 thành Đang vận chuyển'")
        ...
        for tc in tests:
            ...
            if tc["question"].strip().startswith("TODO"):
                print("⏸️ Test Case vẫn còn TODO.")
                todo_count += 1
                continue
        ...
        print("ℹ️ HƯỚNG DẪN:")
        print("  1. python src/app.py --interactive")
        print("  2. python src/app.py --all\n")

        sample_query = tests[1]["question"]
        print("--- 🏁 DEMO TC02: Tra cứu đơn hàng ---")
        logs = run_react_agent(sample_query, provider, mcp_server)
        save_waterfall_trace(logs)
